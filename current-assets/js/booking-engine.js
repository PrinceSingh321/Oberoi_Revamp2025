...
function fnBwSetHotelSelectedByHotelCode(_hotelcode) {
  if (_hotelcode != "") {
    ...
    $(glblBwLastClickedObj)
      .find(".addRoomDropDown .block .roomSelectOption:eq(0)")
      .click();
...
bwMakeReservation.prototype.booknow = function () {
  ...
  if (this.pagename == "sugardoctormedicarellp" && _hotelcode == "5271") {
    ...
    finalUrlWithParameters =
      ...
      "&configcode=" +
      configcode +
      "&brand=ob&chain=24188&level=hotel&locale=" +
      languagForSynxis +
      "&sbe_ri=0" +
      custom_nothn;
  }
  ...
  $(elementSelector).each(function () {
    if ($(this).attr("data-t-date") != "") {
      ...
      //code to check if current selected date is outside disabled date range
        if (
            typeof glblBwConfiguration.offer.disabledDateRange != "undefined"
...
function fnBwSetupBookingWidget(_hotelcode) {
  ...
  if (glblBwConfiguration != null) {
    ...
    $(
      ...
      if (glblBwConfiguration.hotelCity == "Gurgaon" || glblBwConfiguration.hotelCity == "Gurgaon"
          || glblBwConfiguration.hotelCity == "Mumbai" || glblBwConfiguration.hotelCity == "New Delhi" || glblBwConfiguration.hotelCity == "Bengaluru") {
      ...
      if (currentURLPath == "/special-offers/" || currentURLPath == "/")          
       {
          if (isOpenBookingWidgetOfferClicked) { _layout2HotelNameHTML = "Select a hotel"; }
      }
...
function fnBwDisplayMarketingAssetsInCalendar() {
  ...
  if (fnBwBoolShowPricesOnHotels()) {
    if (
      ...
      $(".booking-engine .t-datepicker-days").prepend(
        '<div class="calender-price-msg">Closed due to monsoon in July and August. Reopening on 1st September.</div>'
      );
    } else {
      $(".booking-engine .t-datepicker-days").prepend(
        '<div class="calender-price-msg">Prices are in INR, for a 01 night stay. Better offers may be available for longer stays.</div>'
      );
    }
  }
...
$(document.body).on(
  ...
  function () {
   ...
   $(this).parents(".childAgeWrap").siblings().find(".roomGuestDropDownTxt").removeClass("active");
...
function openBookingEngineOfferSpecific(
  ...
  OfferHeadingName
    ...
    if (offerHotelCode == "" || offerHotelCode == null) { offerHotelCode = glblCurrentPageHotelCode }
  $(".specificOfferName").show();
  if($(".booking-engine.layout2 .hotelName .hotelListDropDown .allHotels .hotelList .hotelBlock.notShowInDropDown .heading:first-child").html() == ""){
...
$(document).click(function(event) {
  if (!$(event.target).closest('.booking-engine .noOfGuestWrap .addGuestDropDown .blockWrapper .block').length) {
      $(".booking-engine .noOfGuestWrap .addGuestDropDown .blockWrapper .block .roomGuestCount .childAgeWrap .ageList").hide();
$(".booking-engine .noOfGuestWrap .addGuestDropDown .roomGuestDropDownTxt").removeClass("active");
$(".booking-engine .noOfGuestWrap .addGuestDropDown .blockWrapper .block .roomGuestCount .choose .number").hide();
$(".booking-engine .noOfGuestWrap .addGuestDropDown .blockWrapper .block .roomGuestCount .choose .label").removeClass("active");
